...
pg_exec() { run_pg psql -h "$PG_HOST" -p "$PG_PORT" -U "$PG_SUPERUSER" \
...
do_backup_postgres() {
    ...
    if run_pg pg_dumpall -h "$PG_HOST" -p "$PG_PORT" -U "$PG_SUPERUSER" \
    ...
    if [[ -n "$db_list" ]]; then
        while IFS= read -r db; do
            ...
            if run_pg pg_dump -h "$PG_HOST" -p "$PG_PORT" -U "$PG_SUPERUSER" \
    ...
    case "$choice" in
        1)
            ...
            if [[ -n "$globals" ]]; then
                ...
                zcat "$globals" | run_pg psql -h "$PG_HOST" -p "$PG_PORT" \
                    -U "$PG_SUPERUSER" --no-password -f - 2>>"$LOG_FILE" || true
            fi

            info "Restoring full cluster ..."
            zcat "$dump" | run_pg psql -h "$PG_HOST" -p "$PG_PORT" \
        ...
        2)
            ...
            if [[ "$chosen" == *.dump ]]; then
                run_pg pg_restore -h "$PG_HOST" -p "$PG_PORT" -U "$PG_SUPERUSER" \
                    --no-password --dbname="$db_name" --verbose --clean --if-exists \
                    "$chosen" 2>>"$LOG_FILE"
            else
                zcat "$chosen" | run_pg psql -h "$PG_HOST" -p "$PG_PORT" \
                    -U "$PG_SUPERUSER" --no-password -d "$db_name" -f - 2>>"$LOG_FILE"
            fi
